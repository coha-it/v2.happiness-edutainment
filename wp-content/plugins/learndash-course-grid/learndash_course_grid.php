...
if ( ! defined( 'LEARNDASH_COURSE_GRID_VERSION' ) ) {
	define( 'LEARNDASH_COURSE_GRID_VERSION', '1.5.1' );
}
...
function learndash_course_grid_localize() {
	...
	load_textdomain( 'learndash-course-grid', WP_LANG_DIR . '/learndash_course_grid/learndash_course_grid-' . $locale . '.mo' );
...
function learndash_course_grid_load_resources() {
	wp_enqueue_style( 'learndash_course_grid_bootstrap', plugins_url( 'assets/css/bootstrap.css', __FILE__ ), array() );
...
add_filter( 'widget_text', 'learndash_course_grid_css' );
function learndash_course_grid_css( $content ) {
	$new_content = preg_replace( '/(.*\[ld_\w+_list.*)/', '<div id="ld_course_list">$1</div>', $content );
